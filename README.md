...
When running parallel subagent fleets (such as orchestrators, workers, explorers, and reviewers), spawning 5–10 agents concurrently on `Model: "inherit"` (Gemini Pro) while outputting raw code rapidly triggers `RESOURCE_EXHAUSTED (code 429)` API quota lockouts.
...
- **Model Tiering**: Subagent workers and reviewers default to `Model: "flash"` or `Model: "flash_lite"`, reserving `pro` strictly for the top-level Master Architect. This delivers 10× higher concurrency limits and prevents 429 lockouts.
