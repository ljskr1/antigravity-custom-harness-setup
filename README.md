...
#### Native `/teamwork-preview` & The `/zen-team` Custom Command
- **Native `/teamwork-preview` (Cloud Gemini 3.8 Flash)**: Runs Google Antigravity's official native cloud multi-agent system out-of-the-box (`TypeName: "teamwork_preview"`, `Model: "inherit"`). It preserves full cloud orchestration with zero hijacking or interference.
- **`/zen-team` (Zero-Token Multi-Agent Fleet)**: Our custom Antigravity slash command mimicking Google's two-phase teamwork protocol:
  - **Phase 1**: Interactive scoping & acceptance criteria in `zen_draft.md`.
  - **Phase 2**: Autonomous execution via **local Zen Router** (`mimo-v2.5-free`, `muse-spark-1.3-contributor-free`) for code scaffolding and **local Ollama** (`deepseek-r1:8b`) for adversarial quality audits.
  - Guarantees **$0.00 worker tokens** and **zero 429 quota exhaustion**.
